...
class AccountsServer {
    ...
    constructor( config: AccountsServerConfiguration ){
        ...
        this.authenticationServices = config.authenticationManager;
    ...
    checkconfig = ( config: AccountsServerConfiguration ) : void => {
        ...
        if( !config.authenticationManager ) 
            throw new AccountsError('[ Accounts - Server ] Init : An Authentication Manager is required');
    }
    ...
    impersonate = async ( accessToken: string, username: string, connectionInfo: ConnectionInformations ) : Promise <ImpersonationResult> => {
        ...
        const isAuthorized: boolean = await this.options.impersonationAuthorize( dbUser, impersonatedUser );
